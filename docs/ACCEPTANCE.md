...
| V08 | 关闭/删除失效（含未发送窗口） | t_rework·T2 clear/admin_off（reset 后真实 OnAgentBegin 窗口失效，首调零泄漏、原输入与哨兵保留）；r3·Q6/Q7；r_rework·RR5；p1·E6；p3·C05/C06/C13-C16；p4·H5 | PASS | PASS |
...
| V11 | 注入去重/上限/不覆盖他人提示；不影响宿主文本类型 | t_rework·T1a-T1d（导入前后 Message.model_validate/rounds_to_text/assemble_context 普通文本一致）；p4·H2c/H3；p2·B01/B09 | PASS | PASS |
...
| V17 | 重载/重启/停用/卸载后恢复 | t_rework·T1f（terminate 后无全局类型残留）；p7·L3（真实 registry activated 停用/恢复过滤）/L5（terminate 后新轮次零注入）；p6·G4；p1·E4 | PASS | PASS |
| V18 | 双版本真实包集成与干净装/卸 | p7·L1（真实 PluginManager.load 完整加载：metadata/config/实例化/12 handler 注册）；p6·G5/G6a；p3_import_check（双钩子注册） | PASS | PASS |
...
- R5 失效覆盖边界：个人 off/clear、管理员总开关、epoch 变化覆盖
  「追加前」「追加后未组装（finalize，-1000 钩子）」「reset 后、首次
  Provider 调用前（on_agent_begin，真实 Agent 钩子）」三道防线
  （t_rework·T2 双动作双版本验证）。**OnAgentBegin 返回后到首次
  Provider 调用之间不存在插件可介入的宿主钩子点（接口缺口）**；插件
  停用/重载发生在途时其钩子可能不再执行，由 finalize 的组装前移除
  与 on_agent_begin 的已完成校验尽量前移覆盖。**已真正发出的请求
  不可撤回**（未把 append/reset/AgentBegin 重定义为发送）。
