# ACCEPTANCE — 验收矩阵映射（九轮证据收尾后 0.1.9）
...
- 证据对应提交：当前候选以 `git log -1` 实查为准（修改代码后本表须重跑）。
- 统计口径：当前 16 脚本每版 264 PASS
  （12+33+28+25+4+21+23+19+12+10+25+16+9+10+9+8）。旧口径订正：0.1.1 候选
  9 脚本每版 190（非 194）；0.1.2 为 211；0.1.3 为 228；0.1.4 为 237；
  0.1.5 为 14 脚本 247；0.1.6/0.1.7 为 15 脚本 256。
- **结论分列（第九轮裁定后）**：
  - **通过**：V01–V10/V12–V20 所列断言；T7 总字符预算（第九轮独立复核
    通过；token_repro 预算两例双版通过、旧 abdba20 重现 140>126）。
  - **未修受阻（非通过）**：T5b 剩余七场景——finalize 后合法钩子复制的
    同文 temp 副本被误删（token_repro 适配版双版各 7 项
    defect_reproduced=true，作为已知受阻失败如实保留，不以删除/跳过
    改绿）；V11 相应标「部分（受阻）」。
  - **接口事实（不计为归属修复）**：转换链四事实（对象身份不跨重建、
    TextPart 模型字段仅 type/text、私有属性不入 dump、同文副本与本尊
    可观测不可区分）由 conversion_chain_probe 与 x_rework·X5 记录，
    是受阻依据而非修复证明。
...
| V15 | 钩子顺序/重试/流式/失败/取消 | w_rework·W3（真实 stop 中止回收）/W4（真实 asyncio 取消，非传播停止）；p7·L1d/L4；r3·Q7；r_rework·RR2；p4·H3、p5·B5；p6·S1/S2/S3 | PASS | PASS |
| V16 | 缺协议先禁用并解释 | r_rework·RR2e/RR2f；p4·H6c/H6d；p5·B6 | PASS | PASS |
| V17 | 重载/重启/停用/卸载后恢复（含在途清理与记录释放） | w_rework·W3/W4（无 AgentDone/decorating 终态的 task-done 回调与弱引用回收）；v_rework·V3（完成归零）/V4（err 终态独立 task 释放）/V6；u_rework·U4/U5；t_rework·T1f；p7·L3/L5；p6·G4；p1·E4 | PASS | PASS |
| V18 | 双版本真实包集成与干净装/卸 | p7·L1（真实 PluginManager.load：15 handler）；u/v/w_rework（真实加载下的完整生命周期，含多步 Agent 与取消）；p6·G5/G6a；p3_import_check | PASS | PASS |
