        ...
        for src, block in extra_pairs:
            for i, item in enumerate(content):
                if item is block:
                    ...
                    # N3：runtime 侧以弱引用登记——映射不延长最终运行时
                    # 实例寿命（宿主侧自身释放规则）；src 侧强引用与请求
                    # 持有的 extra parts 同寿命。失效标记补偿在绑定时刻
                    # 以仍存活的源对象判定（M3）。
